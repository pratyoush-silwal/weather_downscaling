#!/usr/bin/env python3
"""Download ERA5-Land target variables from CDS in monthly NetCDF files."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Download ERA5-Land target fields from CDS.")
...
def main() -> None:
    ...
    output_dir = resolve_path(args.output_dir or cfg["output_dir"])
    ...
    for year, month, days in month_chunks(start, end):
        ...
        target = output_dir / f"era5land_{year}{month:02d}.{suffix}"
        request = {
            "variable": variables,
            "year": [str(year)],
            "month": [f"{month:02d}"],
